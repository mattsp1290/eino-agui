...
The reference app currently uses a local replace:
...
`eino-agui` should add a regression test that fails if those prefixes change, because client
disconnect detection will classify transport errors by those wrappers until the SDK exposes a typed
or sentinel error. If the project upstreams such a sentinel later, replace string matching with
`errors.Is` or an equivalent typed check.
...
During local migration, a temporary sibling checkout replace is acceptable:

```go
replace github.com/ag-ui-protocol/ag-ui/sdks/community/go => /Users/punk1290/git/ag-ui/sdks/community/go
```

That replace must not be committed in a published library release. The published `eino-agui`
